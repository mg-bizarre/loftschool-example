...
function collectDOMStat(root) {
  ...
  for (let child of root.getElementsByTagName('*')) {
    if (child.tagName !== 'SCRIPT') {
      tagArr.push(child.tagName);
      if (child.className != '') {
        child.className.split(' ').map((el) => classArr.push(el));
      }
    }
  }
  ...
  for (let i = 0; i < classArr.length; i++) {
    if (classCounts[classArr[i]]) {
      classCounts[classArr[i]] += 1;
    } else {
      classCounts[classArr[i]] = 1;
    }
  }
  ...
  for (let i = 0; i < tagArr.length; i++) {
    if (tagCounts[tagArr[i]]) {
      tagCounts[tagArr[i]] += 1;
    } else {
      tagCounts[tagArr[i]] = 1;
    }
  }
  ...
  let text = getTextNodesIn(root);

  obj.classes = classCounts;
  obj.tags = tagCounts;
